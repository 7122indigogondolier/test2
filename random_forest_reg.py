...
Team: Utkrist P. Thapa '21
      Abhi Jha '21
      Tina Jin '21
random_forest_reg.py: Implementing a Random Forest Regressor to predict admission probability
"""
...
def main():
    ...
    features = ['GRE', 'TOEFL', 'URATE', 'SOP', 'LOR', 'CGPA',
        ...
        'RACE_white']
    target = 'ADM'
    source_x = df[features]
    y = df[target]

    sc = StandardScaler()
    sc.fit(source_x)
    x = sc.transform(source_x)

    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=1)
    forest = RandomForestRegressor() # using default values
    forest.fit(x_train, y_train)
    print("Score (Coefficient R^2): %.3f" % (forest.score(x_test, y_test)))
