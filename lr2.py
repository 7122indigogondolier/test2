...
class LogisticRegression():
    def __init__(self):
        self.weights = tf.Variable(tf.random.normal([11, 2]))
...
def main():
    ...
    features = ['GRE', 'TOEFL', 'URATE', 'SOP', 'LOR', 'CGPA',
        'RES', 'RACE_Asian', 'RACE_african american', 'RACE_latinx',
        'RACE_white']
    target = 'ADM'
    ...
    print("Test accuracy: %.3f, Precision: %.3f, F-1 score: %.3f" % (accuracy.numpy(),
    precision_score(y_test, predictions), f1_score(y_test, predictions)))
    print("")
    print("-----")
